...
Le but de iperform est de permettre aux utilisateurs de calculer les performances d'une série à une date ou une période donnée. On peut citer par exemple la performance *month-to-date : mtd* (respectivement *year-to-date : ytd*) qui consiste à la somme des valeurs de la série partant du premier jour du mois (respectivement premier jour de l’année civil) jusqu’à date. Le package permet également de présenter un aperçu global d'une série différemment de ce qu'on a par exemple avec la fonction *summary()*.
...
Voici un exmple simple sur l'utilisation de la fonction `mtd()` si l'on veut avoir la performance *month-to-date* en date du *04 août 2023* :
...
* "Les performances" qui renvoient les valeurs de la série en une date ou une la somme sur une période donnée. On cite les fonctions `dday()`, `wtd()`, `mtd()`, `qtd()`, `ytd()` et `full_m()`.

* "Les aperçus" qui renvoient des résumés sur la série en combinant les variations comparées aux périodes antérieures de la série. On cite la fonction `overview()`
