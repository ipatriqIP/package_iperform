...
#' @param a Une valeur de type entier qui permet de décaler l’année par rapport à la date introduite. Sa valeur par defaut est `0`.
...
#' full_y(df, date = "2023-01-08", x = "x")
#'
full_y <- function(data,
                   date,
                   a = 0,
                   x,
                   unite = 1,
  ...
  fin_a <- 365 + ifelse(leap_year(annee), 1, 0)

  if (isTRUE(cumul)) {
    valeur <- data[(data[, "annee"] == annee) & (data[, "jour_a"] == fin_a), x]
    }
  else {
    valeur <- sum(data[data[, "annee"] == annee, x])
    }
